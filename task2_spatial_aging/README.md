...
- [x] **(2)-1** Leiden 클러스터가 뇌 조직의 해부학적 구조  
      (cortex 층 등) 를 얼마나 반영하는지 시각적 대조
- [ ] **(2)-2** 기존 `cell_type` vs Leiden 일치도 평가 (ARI/NMI)  
      + 불일치 클러스터 원인 추정
- [x] **(2)-3** `age` 활용, Young vs Old 마우스 간 특정 세포 유형  
      (예: Microglia) 의 공간분포·클러스터 비율 변화 정량 비교

### (3) 데이터간 비교
- [x] **(3)-1** 노화(Old) vs LPS 두 샘플 공통 발현 변화 유전자 선별
- [x] **(3)-2** 선별 유전자군을 Cell-type map 위에 시각화 비교
